...
local function parse_client_data(object_name, faces, materials, shaders)
	...
	if mtl then
		...
		for i = 1, #mtl_split do
			...
			if not data[2] then continue end	-- ignore empty lines
		...
		print("Couldn't find .mtl file when parsing " .. object_name .. "not ")
	end
	...
	for i = 1, #faces do
		...
		if faces[i] and #faces[i] / 3 > 21845 then 
			print("Failed to parse face " .. i .. " as it has " .. #faces[i] / 3 .. " trianglesnot  (Limit of 21,845)")
		end
...
local function unfuck_negative(v_str, max)
	if not v_str or v_str == "" then return 0 end
...
function InfMap.parse_obj(object_name, translation, client_only, shaders)
	...
	if not obj then 
		print("Couldn't find .obj file when parsing " .. object_name .. "not  (is the file in maps/ ?)")
		return 
	end
	...
	local coro = coroutine.create(function()
		...
		for i = 1, split_obj_len do
			...
			elseif first == "f" then 
				...
				if not faces[material] then
					print("Material undefined for group " .. group)
					material = material + 1
					faces[material] = {}
				end
	
				-- who tf uses negative indexes?not ??
				-- why am I adding support for thisnot ?
				...
				for i = 3, #line_data do
					...
					--if not vertex1_pos or not vertex2_pos or not vertex3_pos then continue end
	...
	if SERVER then
		...
		timer.Simple(0, function()	-- race condition
			...
			for _, collider in ipairs(ents.FindByClass("infmap_obj_collider")) do
				...
				if not collider.UpdateCollision then
