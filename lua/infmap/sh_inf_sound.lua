...
local function IsValidLoop( data )
	...
	return a ~= c and b ~= c and not (e and f) and not data.Entity:IsVehicle() --is valid loop sound?
end
...
if SERVER then
	...
	hook.Add( "EntityEmitSound", "infmap_sounddetour", function(data)
		if data.Entity:IsWorld() then --emitting sounds from world is stupid, lets change to entity
			...
			for k, v in pairs(ents.GetAll()) do
				...
				if vec:DistToSqr(data.Pos) < dist and not InfMap.filter_entities(v) then --get closest entity to emitted sound, filterednot 
		...
		for a, b in pairs( inf_sounds_store ) do
			for c, d in pairs( b ) do
				for e, f in pairs( d ) do
					if not IsValid(c) then table.RemoveByValue(b,d) continue end
					if CheckDistance( a, c, 1 ) then
						...
						net.Send(a)
						table.RemoveByValue(b,d) --only send sound oncenot 
					end
				end
			end
		end
	...
	net.Receive( "inf_ent_networksound", function()
		...
		if IsValid(data.Entity) then
			if IsLoop(data) and not IsValidLoop(data) and data.Entity:GetParent() ~= LocalPlayer() then --check if sound is awkward looping sound
				...
				if game.SinglePlayer() or not (data.Entity == LocalPlayer() and IsPlayerSound(data)) or valid_sounds[data.OriginalSoundName] then --exception for players, sounds seem to duplicate for them
					if not IsValid(inf_csounds[data.Entity]) then
	...
	hook.Add( "Think", "infmap_soundmanager", function()
		for k, v in pairs(inf_sounds) do
			if not IsValid(k) then table.RemoveByValue(inf_sounds,k) continue end
			if not CheckDistance(LocalPlayer(), v.Entity, 1) then
				v.Entity:StopSound(v.OriginalSoundName) --stop sound if too far away
			end
		end
		for k, v in pairs(inf_csounds) do
			if not IsValid(v) or not IsValid(k) then table.RemoveByValue(inf_csounds,k) continue end
	...
	InfMap.CreateSound = InfMap.CreateSound or CreateSound
	function CreateSound(ent, soundname, filter)
		if not IsValid(inf_csounds[ent]) then inf_csounds[ent] = SoundObject(ent) end
