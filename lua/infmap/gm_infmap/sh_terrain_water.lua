...
hook.Add("CalcMainActivity", "infmap_water_swimming", function(ply)
	if not inWater(ply:GetPos()) or ply:IsOnGround() or ply:InVehicle() then return end
	return ACT_MP_SWIM, -1
end)

-- main movement
hook.Add("Move", "infmap_water_swimming", function(ply, move)
	if not inWater(ply:GetPos()) then return end
...
if SERVER then
	...
	hook.Add("Think", "infmap_water_buoyancy", function()
		...
		for _, prop in ipairs(entities) do
			...
			if not phys:IsValid() or phys:IsAsleep() then continue end
			...
			if valid_materials[phys:GetMaterial()] or is_airboat then 
				...
				local should_sleep = (phys:GetVelocity() + phys:GetAngleVelocity()):Length() < 1 and not prop:IsPlayerHolding()
