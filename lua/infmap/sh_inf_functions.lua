...
function InfMap.in_chunk(pos, size) 
	...
	return not (pos[1] <= -cs or pos[1] >= cs or pos[2] <= -cs or pos[2] >= cs or pos[3] <= -cs or pos[3] >= cs)
end
...
function InfMap.filter_entities(e)
	...
	--if not e.GetModelRenderBounds and not e:GetModelRenderBounds() then return true end
...
local function constrained_invalid_filter(ent)
	...
	if phys:IsValid() then
		phys_filter = not phys:IsMoveable()	-- filter frozen props
	end
	return InfMap.filter_entities(ent) or (not ent:IsSolid() and ent:GetNoDraw()) or ent:GetParent():IsValid() or phys_filter or (ent:IsWeapon() and ent:GetOwner():IsValid())
end
...
function InfMap.constrained_status(ent) 
	...
	for _, constrained_ent in ipairs(ent.CONSTRAINED_DATA) do
		...
		if constrained_ent:EntIndex() < ent_index and not constrained_invalid_filter(constrained_ent) then 
			ent.CONSTRAINED_MAIN = false
			return ent.CONSTRAINED_MAIN
		end
...
function InfMap.split_convex(tris, plane_pos, plane_dir)
	...
	for i = 1, #tris, 3 do
		...
		--if not pos1 or not pos2 or not pos3 then continue end	  -- just in case??
		...
		if not pos1_valid and not pos2_valid and not pos3_valid then 
			continue 
		end
		...
		if pos1_valid then
			if pos2_valid then	  --pos1 = valid, pos2 = valid, pos3 = invalid
				...
				if not point1 then point1 = pos3 end
				if not point2 then point2 = pos3 end
			...
			elseif pos3_valid then  -- pos1 = valid, pos2 = invalid, pos3 = valid
				...
				if not point1 then point1 = pos2 end
				if not point2 then point2 = pos2 end
		...
		elseif pos2_valid then
			if pos3_valid then	  -- pos1 = invalid, pos2 = valid, pos3 = valid
				...
				if not point1 then point1 = pos1 end
				if not point2 then point2 = pos1 end
