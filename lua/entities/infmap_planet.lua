...
function ENT:Initialize()
	if CLIENT then 
		if not self.CHUNK_OFFSET then return end
...
function ENT:Think()
	if not IsValid(self:GetPhysicsObject()) and self.CHUNK_OFFSET then
...
function ENT:GetRenderMesh()
	local self = self
	if not self.RENDER_MESH then return end

	-- No Trees?
	if not self.TreeMatrices then return self.RENDER_MESH end
