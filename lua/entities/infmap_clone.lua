...
function ENT:InitializeClient(parent)
	...
	if not phys:IsValid() then -- no custom physmesh, bail
		self:PhysicsInit(SOLID_VPHYSICS)
		return 
	end

	local convexes = phys:GetMesh()
	if not convexes then   -- no convexes, bail
		self:PhysicsInit(SOLID_VPHYSICS)
		return
	end
...
function ENT:Think()
	...
	if not IsValid(parent) or data.chunk_offset ~= parent.CHUNK_OFFSET + data.chunk then
		SafeRemoveEntity(self)
		return
	end
